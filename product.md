...
1. 配对的两个单词变灰，保持在页面上
2. 倒计时立即重置为 60 秒
3. 延迟 2 秒后，配对成功的单词淡出消失，形成空位
4. 新单词随机填充到空位中（中文和英文分别独立随机选择空位位置）
5. UI 更新采用平滑动画过渡，无闪烁闪动
6. 在这 2 秒延迟期间，用户可以继续配对其他剩余单词（不影响操作）
7. 记录正确次数，更新熟练度

**重要说明**：新单词补充时的随机性机制：
- 中文新词会从左列的所有空位中随机选择一个位置
- 英文新词会从右列的所有空位中随机选择一个位置
- 中英文位置是独立随机的，不会始终出现在相同的行
- 这种设计增加了游戏的随机性和挑战性，避免了新单词总是出现在固定位置的问题
